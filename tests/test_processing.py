    ...
    def test_v1_workflow_writes_expected_artifacts_with_mock_search(self):
        ...
        try:
            ...
            with TemporaryDirectory() as tmp:
                ...
                self.assertTrue((args.output_dir / "planning" / "prompt_manifest.json").exists())
                self.assertTrue((args.output_dir / "planning" / "registries.json").exists())
                self.assertTrue((args.output_dir / "planning" / "protocol.json").exists())
                self.assertTrue((args.output_dir / "planning" / "user_corpus_log.json").exists())
                self.assertTrue((args.output_dir / "search" / "metadata.json").exists())
                self.assertTrue((args.output_dir / "search" / "source_diagnostics.json").exists())
                self.assertTrue((args.output_dir / "corpus" / "corpus.json").exists())
                self.assertTrue((args.output_dir / "corpus" / "ranked_papers.json").exists())
                self.assertTrue((args.output_dir / "verification" / "verification.json").exists())
                self.assertTrue((args.output_dir / "verification" / "download_log.json").exists())
                self.assertTrue((args.output_dir / "verification" / "paper_notes.json").exists())
                self.assertTrue((args.output_dir / "verification" / "quality_gate.json").exists())
                self.assertTrue((args.output_dir / "verification" / "evidence_ledger.json").exists())
                self.assertTrue((args.output_dir / "verification" / "review_agent_findings.json").exists())
                self.assertTrue((args.output_dir / "synthesis" / "literature_matrix.json").exists())
                self.assertTrue((args.output_dir / "synthesis" / "synthesis.json").exists())
                self.assertTrue((args.output_dir / "reports" / "report.canonical.json").exists())
                self.assertTrue((args.output_dir / "reports" / "report.zh.html").exists())
                self.assertTrue((args.output_dir / "reports" / "report.en.html").exists())
                self.assertTrue((args.output_dir / "reports" / "report.zh.pdf").exists())
                self.assertTrue((args.output_dir / "wiki" / "obsidian" / "index.md").exists())
                self.assertTrue((args.output_dir / "wiki" / "obsidian" / "_meta" / "manifest.json").exists())
                self.assertGreaterEqual(len(list((args.output_dir / "wiki" / "obsidian" / "papers").glob("*.md"))), 35)
                self.assertFalse((args.output_dir / "report.zh.html").exists())
                self.assertFalse((args.output_dir / "metadata.json").exists())
                self.assertFalse((args.output_dir / "download_log.json").exists())
                html = (args.output_dir / "reports" / "report.zh.html").read_text(encoding="utf-8")
                self.assertIn("研究背景与问题定义", html)
                self.assertIn("代表论文总结", html)
                self.assertIn("证据账本与复核 Agent", html)
                ledger = json.loads((args.output_dir / "verification" / "evidence_ledger.json").read_text(encoding="utf-8"))
                self.assertGreater(ledger["claim_count"], 0)
                review = json.loads((args.output_dir / "verification" / "review_agent_findings.json").read_text(encoding="utf-8"))
                ...
                corpus = json.loads((args.output_dir / "corpus" / "corpus.json").read_text(encoding="utf-8"))
                ...
                ranked = json.loads((args.output_dir / "corpus" / "ranked_papers.json").read_text(encoding="utf-8"))
                self.assertGreaterEqual(len(ranked), 35)
                lint = json.loads((args.output_dir / "wiki" / "obsidian" / "_meta" / "wiki_lint.json").read_text(encoding="utf-8"))
    ...
    def test_v1_workflow_explicit_minimum_shortfall_does_not_write_formal_report(self):
        ...
        try:
            ...
            with TemporaryDirectory() as tmp:
                ...
                self.assertEqual(code, 2)
                self.assertTrue((args.output_dir / "verification" / "shortfall.json").exists())
                self.assertFalse((args.output_dir / "reports" / "report.canonical.json").exists())
                self.assertFalse((args.output_dir / "wiki" / "obsidian" / "index.md").exists())
        finally:
            workflow_module.search_all = original_search_all
